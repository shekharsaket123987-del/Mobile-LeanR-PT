...
| 2 — Authentication | ✅ Done | Email/password login, an email-OTP flow (`(auth)/otp.tsx`), password reset (`forgot-password.tsx`/`reset-password.tsx`, deep-link-driven recovery session per `auth-context.tsx`), and Google OAuth (`signInWithGoogle`, Supabase's web-based OAuth pattern via `expo-web-browser` — no client-side Google credential needed, GoTrue holds it) are all implemented, plus role-based routing (`lib/auth/role-routing.ts`). **One external dependency remains**: Google sign-in's code path is complete, but it only functions once the Google provider is turned on in the Supabase dashboard (Authentication → Providers → Google) with a Google Cloud OAuth client configured there — a credential/config step outside this repo, not missing code. |
...
**Net effect:** the roadmap in this section is ~90% executed for client + coach portals, auth, and push infra; the one item worth carrying forward is that Google OAuth's code path is done but inert until the Supabase dashboard's Google provider is configured with a real Google Cloud OAuth client — and admin parity was intentionally capped (§Phase 12), not deferred by accident. Testing has a real but partial start (unit-level business rules only); everything requiring a live device/backend remains open. Store release now has a real branded icon; the remaining gate is external accounts (EAS/Apple/Google), not more app code.
...
3. ~~**Cancellation session-credit refund**~~ — **Resolved 2026-08-28**, confirmed live via direct `pg_proc` introspection of `cancel_booking()`'s body: there is no session-credit deduction at booking time to refund in the first place. `subscriptions` has no stored `sessions_remaining` column (confirmed via `information_schema.columns` — only `sessions_total` is stored); "remaining" is always computed as `sessions_total - count(bookings where status='completed')`. `cancel_booking()` only sets `status='cancelled'`/`cancelled_by`/`cancel_reason` and regenerates the next recurring occurrence if applicable — it never touches `subscriptions`. Since a cancelled booking (by definition never `completed`) was never counted against the total, cancellation needs no refund logic, and none exists. The mobile app's own `subscription.ts` already computes "remaining" the same way (counts `status='completed'`), so no mobile code change was needed — this was a documentation-only resolution.
