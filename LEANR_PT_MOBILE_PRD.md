...
10. ~~**Escalation/coach-change "reopen" controls**~~ — **Resolved 2026-08-30**, confirmed live via `pg_trigger`/`pg_constraint` introspection on both tables: no trigger and no CHECK constraint enforces a terminal `status` on either `escalations` or `coach_change_requests` (only `set_updated_at`/audit-log triggers, plus unrelated CHECKs on `category`/`fault`/rating ranges). "No reopen" is purely a UI convention — no screen offers the control — not a DB-enforced rule. An admin with ordinary UPDATE access could technically set `status` back if a future screen were built to do it; nothing blocks it today.
