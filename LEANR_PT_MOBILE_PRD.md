...
| 14 — Store Release | 🟡 Scaffolding only | ESLint + EAS/store config scaffolding exists (per commit history); no evidence of an actual TestFlight/Play internal build yet. |

**Net effect:** the roadmap in this section is ~90% executed for client + coach portals, auth, and push infra; the one item worth carrying forward is that Google OAuth's code path is done but inert until the Supabase dashboard's Google provider is configured with a real Google Cloud OAuth client — and admin parity was intentionally capped (§Phase 12), not deferred by accident. Testing has a real but partial start (unit-level business rules only); everything requiring a live device/backend remains open.
