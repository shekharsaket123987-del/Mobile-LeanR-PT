...
**Still open against this document specifically:** auth is now email/password + email-OTP + password reset + Google OAuth, all fully live (the "Continue with Google" affordance from original PRD §25 auth navigation works end-to-end — Google's provider was verified live 2026-08-30 via a direct auth-endpoint check, not just toggled on). Testing (§29 of the original PRD) has a real but partial start — a Jest suite covers pure business-rule logic (IST/timezone math, reschedule cutoff, recurring-schedule hour-matching, the auth deep-link parser, streaks, ratings, chat summarization); everything needing a live device, simulator, or backend (payments, Zoom, Realtime, push delivery, RLS boundaries) is still open. Store release is not started.
