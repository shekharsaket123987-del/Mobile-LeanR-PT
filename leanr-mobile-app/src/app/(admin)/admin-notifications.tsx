/**
 * Notifications (admin) — New PRD.md §4.C "Screen: Notifications" —
 * reuses the same shared `notifications.ts` data layer the client/coach
 * portals use verbatim (New PRD.md: "Reuses the client portal's
 * NotificationsClient verbatim"), routed to admin's own screens.
 */
...
const ADMIN_ROUTES: Record<string, '/admin-sessions' | '/admin-escalations' | '/coach-change-requests'> = {
  sessions: '/admin-sessions',
  concerns: '/admin-escalations',
  coach: '/coach-change-requests',
};
...
export default function AdminNotificationsScreen() {
  ...
  const onPress = async (notification: NotificationRow) => {
    ...
    const target = category ? ADMIN_ROUTES[category] : null;
